...
describe("Subtasks & Acceptance Criteria", () => {
  ...
  it("should create acceptance criteria", async () => {
    const res = await app.inject({
      method: "POST",
      url: `/api/cards/${cardId}/acceptance-criteria`,
      payload: { description: "Tests pass with 80% coverage" },
    });

    expect(res.statusCode).toBe(201);
    const ac = JSON.parse(res.body);
    expect(ac.description).toBe("Tests pass with 80% coverage");
    expect(ac.status).toBe("pending");
    expect(ac.cardId).toBe(cardId);
  });

  it("should list acceptance criteria for a card", async () => {
    await app.inject({
      method: "POST",
      url: `/api/cards/${cardId}/acceptance-criteria`,
      payload: { description: "Criterion 1", position: 0 },
    });
    await app.inject({
      method: "POST",
      url: `/api/cards/${cardId}/acceptance-criteria`,
      payload: { description: "Criterion 2", position: 1 },
    });
    ...
    expect(res.statusCode).toBe(200);
    const criteria = JSON.parse(res.body);
    expect(criteria).toHaveLength(2);
  });

  it("should set acceptance criteria status to pass", async () => {
    const createRes = await app.inject({
      method: "POST",
      url: `/api/cards/${cardId}/acceptance-criteria`,
      payload: { description: "Should work" },
    });
    const ac = JSON.parse(createRes.body);
    ...
    expect(res.statusCode).toBe(200);
    expect(JSON.parse(res.body).status).toBe("pass");
  });

  it("should set acceptance criteria status to fail", async () => {
    const createRes = await app.inject({
      method: "POST",
      url: `/api/cards/${cardId}/acceptance-criteria`,
    ...
    expect(res.statusCode).toBe(200);
    expect(JSON.parse(res.body).status).toBe("fail");
  });
